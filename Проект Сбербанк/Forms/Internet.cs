...
namespace Проект_Сбербанк.Forms
{
    public partial class Internet : Form
    {
        ...
        private void button_Pay_Click(object sender, EventArgs e)
            {
                ...
                if (sum < 0)
                {
                MessageBox.Show("Ошибка. Сумма перевода должна быть больше 1 тенге", "Отмена", MessageBoxButtons.OK, MessageBoxIcon.Information);
                error = true;
                }
                ...
                if (error == false)
                {
                    ...
                    if (DataStorage.attempts > 0)
                    {
                        ...
                        var queryTransaction2 = $"insert into transactions (transaction_type, transaction_destination, transaction_date, transaction_number, transaction_value, id_bank_card) values ('Internet Payments', '{comboBox1.GetItemText(comboBox1.SelectedItem)}', '{transactionDate}', '{transactionNumber}', '{sum}', (select id_bank_card from bank_card where bank_card_number = '{cardNumber}'))";
                        var queryTransaction3 = $"update clientServices set serviceBalance = serviceBalance + '{sum}' where serviceName = '{comboBox1.GetItemText(comboBox1.SelectedItem)}' and serviceType = 'Internet'";
                        ...
                        var queryTransaction5 = $"insert into project(account_number, category_contribution, passport_details, current_deposit_amount, last_transaction_date) values('{DataStorage.idClient}', 'Internet Payments', '{passport}', '{sum}', '{transactionDate}')";
        ...
        private void Internet_MouseMove(object sender, MouseEventArgs e)
        {
        ...
        private void Internet_MouseDown(object sender, MouseEventArgs e)
        {
